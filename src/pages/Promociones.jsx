  ...
  return (
    <div className="page">
      ...
      <section className="section container promotions-section">
        ...
        {!state.loading && !state.error && !(state.data || []).length ? <PromotionParticipation /> : null}
        {!state.loading && !state.error && state.data?.length ? <div className="mosaic">{state.data.map((promotion) => <PromotionCard key={promotion.CodigoPromocion} promotion={promotion} />)}</div> : null}
      </section>
    </div>
  );
}
