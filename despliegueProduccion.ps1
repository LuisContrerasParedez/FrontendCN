...
. (Join-Path $projectRoot 'scripts\Deploy.Produccion.Common.ps1')

$deployPath = Join-Path $projectRoot 'deploy-produccion'
$workPath = Join-Path $projectRoot '.deploy-produccion-work'
$statePath = Join-Path $projectRoot '.tools\despliegueProduccion.state.json'
$version = (Get-Date).ToString('yyyyMMdd-HHmmss')
$packageName = "centranorte-produccion-$version.tar.gz"
...
for persistent in imagenes; do
    ...
    if [ -d "$staging/$persistent" ] && find "$staging/$persistent" -type l -print -quit | grep -q .; then
        echo "El contenido persistente contiene enlaces simbolicos no permitidos: $persistent" >&2
        exit 24
    fi
done
...
try {
    Write-Host "Preparando despliegue de produccion $version" -ForegroundColor Cyan
    $config = Get-DeploymentConfig -ProjectRoot $projectRoot

    if ($ValidateConfigOnly) {
        Write-Host ("Configuracion de produccion valida: {0} -> {1}" -f $config.PublicUrl, $config.RemotePublicPath) -ForegroundColor Green
        return
    }
